...
pub fn start(){
    ...
    //
    // let entry:&Entry = entries.get(0).unwrap();
    // let count = entry.maps.get(0).unwrap().sources.len();
    // println!("Size:{}",count);
    // let seed:&u64 = entry.seeds.get(0).unwrap();
    // let out = entry.passthrough(79);


    // entries.iter().for_each(|mut entry|{
    //     entry.go();
    //     //println!("{} -> {}",seed, out);
    // });
}